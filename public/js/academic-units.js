...
function populateDepartmentSelect(deptSelect, faculty, placeholder = 'Bölüm seçin') {
  if (!deptSelect) return;

  deptSelect.innerHTML = '';
  deptSelect.disabled = !faculty;

  const defaultOption = document.createElement('option');
  defaultOption.value = '';
  defaultOption.disabled = true;
  defaultOption.selected = true;
  defaultOption.textContent = faculty ? placeholder : 'Önce fakülte seçin';
  deptSelect.appendChild(defaultOption);

  if (!faculty) return;

  getDepartmentsForFaculty(faculty).forEach(department => {
    const option = document.createElement('option');
...
function initFacultyDepartmentSelectors(facultySelectId, departmentSelectId) {
  ...
  facultySelect.addEventListener('change', () => {
    populateDepartmentSelect(departmentSelect, facultySelect.value);
  });
}
